...
const db = new sqlite3.Database(path.join(__dirname, "akihabara.db"));
...
function normalizeUsername(username) {
    const cleaned = String(username || "")
        .trim()
        .replace(/^@/, "");
    return isValidUsername(cleaned) ? cleaned : "";
}
...
// Строгий middleware проверки актуальных прав администратора через БД
...
app.post("/login", (req, res) => {
    ...
    if (!username) {
        return res.status(400).json({
            ok: false,
            error: "Введите корректный username (допустимы латиница, цифры, _, -, . от 3 до 32 символов)"
        });
    }
...
app.post("/api/admin/telegram-chat", requireAdmin, (req, res) => {
...
app.post("/api/admins/add", requireAdmin, (req, res) => {
...
app.post("/api/admins/remove", requireAdmin, (req, res) => {
...
app.post("/api/admins/edit", requireAdmin, (req, res) => {
...
app.post("/api/exchangers/add", requireAdmin, (req, res) => {
...
app.post("/api/exchangers/edit", requireAdmin, (req, res) => {

    const id = Number(req.body.id);
    const username = normalizeUsername(req.session.username);

    const query = req.userRole === "main_admin"
        ? "UPDATE exchangers SET name = ?, photoUrl = ?, telegramUrl = ?, description = ? WHERE id = ?"
        : "UPDATE exchangers SET name = ?, photoUrl = ?, telegramUrl = ?, description = ? WHERE id = ? AND owner = ?";

    const params = req.userRole === "main_admin"
        ? [String(req.body.name || ""), String(req.body.photoUrl || ""), String(req.body.telegramUrl || ""), String(req.body.description || ""), id]
        : [String(req.body.name || ""), String(req.body.photoUrl || ""), String(req.body.telegramUrl || ""), String(req.body.description || ""), id, username];
    ...
    db.run(query, params, function(err) {
        ...
        if (this.changes === 0) {
            return res.status(403).json({ ok: false, error: "Объект не найден или нет прав" });
        }
...
app.post("/api/exchangers/delete", requireAdmin, (req, res) => {
    ...
    const query = req.userRole === "main_admin"
        ...
        : "DELETE FROM exchangers WHERE id = ? AND owner = ?";

    const params = req.userRole === "main_admin" ? [id] : [id, username];
...
app.post("/api/shops/add", requireAdmin, (req, res) => {
...
app.post("/api/shops/edit", requireAdmin, (req, res) => {

    const id = Number(req.body.id);
    const username = normalizeUsername(req.session.username);

    const query = req.userRole === "main_admin"
        ? "UPDATE shops SET name = ?, photoUrl = ?, telegramUrl = ?, description = ? WHERE id = ?"
        : "UPDATE shops SET name = ?, photoUrl = ?, telegramUrl = ?, description = ? WHERE id = ? AND owner = ?";
...
app.post("/api/shops/delete", requireAdmin, (req, res) => {
...
.logo { width: 100%; max-height: 180px; object-fit: contain; margin-bottom: 15px; }
h1 { font-size: 26px; margin: 10px 0 25px; }
input {
    width: 100%; padding: 15px; border-radius: 14px; border: 1px solid #444;
    background: #151515; color: white; outline: none; margin-bottom: 12px;
}
button {
    width: 100%; padding: 14px; border: 0; border-radius: 14px;
    background: linear-gradient(135deg,#ff1744,#8e24aa); color: white;
...
.header { position: sticky; top: 0; z-index: 100; background: rgba(5,5,5,.94); backdrop-filter: blur(15px); border-bottom: 1px solid #292929; }
...
.stat { flex: 1; min-width: 90px; background: #151515; padding: 9px; border-radius: 12px; text-align: center; font-size: 13px; }
...
.tab { background: #171717; color: white; border: 1px solid #333; padding: 13px 5px; border-radius: 13px; cursor: pointer; }
.tab.active { background: linear-gradient(135deg,#ff1744,#8e24aa); }
.page { display: none; }
.page.active { display: block; }
.chat { min-height: 500px; background: rgba(8,8,8,.88); border-radius: 20px; padding: 15px; border: 1px solid #292929; }
.messages { min-height: 400px; max-height: 60vh; overflow-y: auto; }
.message { display: flex; gap: 10px; margin-bottom: 13px; }
.avatar { width: 40px; height: 40px; min-width: 40px; border-radius: 50%; object-fit: cover; background: #222; }
.message-body { max-width: calc(100% - 55px); }
.username { color: #ff4770; font-weight: bold; margin-bottom: 3px; }
.text { background: #181818; border: 1px solid #2d2d2d; padding: 10px 12px; border-radius: 13px; word-wrap: break-word; }
.message img.media { max-width: 280px; max-height: 300px; border-radius: 12px; display: block; margin-top: 7px; }
...
.small-btn { border: 0; background: #252525; color: #ddd; padding: 5px 9px; border-radius: 8px; cursor: pointer; }
.composer { margin-top: 15px; }
textarea { width: 100%; min-height: 70px; resize: vertical; background: #111; color: white; border: 1px solid #333; border-radius: 14px; padding: 12px; }
.media-row { display: flex; gap: 8px; margin-top: 8px; }
.media-row input { flex: 1; background: #111; color: white; border: 1px solid #333; border-radius: 10px; padding: 10px; }
.send { margin-top: 8px; width: 100%; padding: 12px; background: linear-gradient(135deg,#ff1744,#8e24aa); border: 0; border-radius: 12px; color: white; font-weight: bold; cursor: pointer; }
.cards { display: grid; grid-template-columns: repeat(auto-fit,minmax(250px,1fr)); gap: 15px; }
.card { background: rgba(15,15,15,.92); border: 1px solid #333; border-radius: 18px; padding: 15px; }
.card img { width: 100%; height: 170px; object-fit: cover; border-radius: 13px; margin-bottom: 10px; }
.card h3 { margin: 5px 0 8px; }
.card p { color: #bbb; }
.card a { display: inline-block; padding: 10px 14px; background: #1689d8; color: white; text-decoration: none; border-radius: 10px; }
...
.admin { margin-top: 20px; background: rgba(15,15,15,.95); padding: 20px; border: 1px solid #444; border-radius: 20px; }
.admin-section { margin-bottom: 25px; }
.admin input, .admin textarea { width: 100%; margin-bottom: 8px; background: #111; color: white; border: 1px solid #333; padding: 11px; border-radius: 10px; }
.admin button { margin-bottom: 7px; }
.admin-item { background: #191919; padding: 12px; border-radius: 12px; margin-bottom: 8px; }
.danger { background: #b71c1c !important; border: 0; color: white; padding: 8px 12px; border-radius: 8px; cursor: pointer; }
...
<div class="media-row">
<input id="mediaUrl" placeholder="Ссылка на фото/видео (необязательно)">
...
<input id="avatarUrl" placeholder="URL вашей аватарки" style="width:100%;padding:12px;background:#111;color:white;border:1px solid #333;border-radius:10px">
...
<input id="exchangerPhoto" placeholder="URL фото">
...
async function loadUser() {
    ...
    document.getElementById("refLink").value = location.origin + "/?ref=" + encodeURIComponent(currentUser.referralCode || currentUser.username);
...
async function loadTelegram() {
    ...
    const data = await r.json();
    document.getElementById("telegramButton").href = data.url;
    ...
    div.innerHTML = \`
        ...
        <div class="message-body">
            ...
            <div class="message-actions">
                ...
                \${currentUser && (currentUser.status === "admin" || currentUser.status === "main_admin") ? \`<button class="small-btn" onclick="pinMessage(\${message.id})">📌</button>\` : ""}
            </div>
        </div>
    \`;
...
function escapeJs(value) { return String(value || "").replace(/\\\\/g,"\\\\\\\\").replace(/'/g,"\\\\'").replace(/"/g,'\\\\"'); }
...
socket.on("message_liked", () => { location.reload(); });
socket.on("message_pinned", () => { location.reload(); });
socket.on("system_message", data => {
    const div = document.createElement("div");
    div.style.textAlign = "center"; div.style.color = "#888"; div.style.margin = "10px";
...
async function loadExchangers() {
    ...
    data.exchangers.forEach(item => {
        box.innerHTML += \`
        <div class="card">
            \${item.photoUrl ? \`<img src="\${escapeAttr(item.photoUrl)}">\` : ""}
            <h3>\${escapeHtml(item.name)}</h3>
            <p>\${escapeHtml(item.description)}</p>
            \${item.telegramUrl ? \`<a href="\${escapeAttr(item.telegramUrl)}" target="_blank">✈️ Telegram</a>\` : ""}
            \${currentUser && (currentUser.status === "admin" || currentUser.status === "main_admin") ? \`<br><br><button onclick="editExchanger(\${item.id})">✏️ Редактировать</button> <button class="danger" onclick="deleteExchanger(\${item.id})">🗑 Удалить</button>\` : ""}
        </div>\`;
    });
}
...
async function loadShops() {
    ...
    data.shops.forEach(item => {
        ...
        <div class="card">
            ...
            <p>\${escapeHtml(item.description)}</p>
            \${item.telegramUrl ? \`<a href="\${escapeAttr(item.telegramUrl)}" target="_blank">✈️ Telegram</a>\` : ""}
            \${currentUser && (currentUser.status === "admin" || currentUser.status === "main_admin") ? \`<br><br><button onclick="editShop(\${item.id})">✏️ Редактировать</button> <button class="danger" onclick="deleteShop(\${item.id})">🗑 Удалить</button>\` : ""}
        </div>\`;
    });
}
...
async function loadAdmins() {
    ...
    data.admins.forEach(admin => {
        ...
        <div class="admin-item">
            <strong>@\${escapeHtml(admin.username)}</strong><br>Статус: \${escapeHtml(admin.status)}
            \${admin.status !== "main_admin" ? \`<br><br><button onclick="editAdmin('\${escapeJs(admin.username)}')">✏️ Изменить</button> <button class="danger" onclick="removeAdmin('\${escapeJs(admin.username)}')">🗑 Удалить права</button>\` : ""}
        </div>\`;
    });
}
...
async function editShop(id) {
    ...
    const r = await fetch("/api/shops/edit", {
        method:"POST",
        headers:{ "Content-Type":"application/json" },
        body:JSON.stringify({ id, name, photoUrl, telegramUrl, description })
    });
    const data = await r.json();
    if (!data.ok) { alert(data.error || "Ошибка"); return; }
    loadShops();
...
async function loadUsers() {
    ...
    allUsers.forEach(user => {
        box.innerHTML += \`
        <div class="admin-item">
            <strong>@\${escapeHtml(user.username)}</strong><br>Статус: \${escapeHtml(user.status)}<br>Рефералов: \${user.invites || 0}
            \${user.username !== "koliaegorov99po-afk" ? \`<br><br><button onclick="changeUserStatus('\${escapeJs(user.username)}','\${user.status === "admin" ? "user" : "admin"}')">\${user.status === "admin" ? "👤 Сделать пользователем" : "👑 Сделать админом"}</button> <button class="danger" onclick="deleteUser('\${escapeJs(user.username)}')">🗑 Удалить</button>\` : ""}
        </div>\`;
    });
}
...
async function loadComplaints() {
    ...
    (data.complaints || []).forEach(c => {
        box.innerHTML += \`
        <div class="admin-item">
            <strong>\${escapeHtml(c.target_name)}</strong><br>От: @\${escapeHtml(c.complainant)}<br>Причина: \${escapeHtml(c.reason)}
        </div>\`;
    });
}
...
async function saveAvatar() {
    ...
    const data = await r.json();
    if (!data.ok) { alert("Ошибка"); return; }
