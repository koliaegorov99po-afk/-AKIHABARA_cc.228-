...
<div class="stat">
👑
<span id="adminName">Главный админ</span>
</div>
...
<button class="tab"
onclick="switchTab('exchangers',this)">
💱 ОБМЕННИКИ
</button>

<button class="tab"
onclick="switchTab('shops',this)">
🏪 МАГАЗИНЫ
</button>
...
<p>
Рефералы:
<strong id="profileInvites">0</strong>
</p>
...
<button onclick="updateTelegramChat()">
СОХРАНИТЬ TELEGRAM
</button>
...
<input
id="newAdmin"
placeholder="username нового администратора">
...
async function loadUser() {
    ...
    document.getElementById("profileInvites").textContent =
    ...
    const link =
        location.origin +
        "/?ref=" +
        encodeURIComponent(currentUser.referralCode || currentUser.username);
...
async function loadStats() {
    ...
    document.getElementById("totalRefs").textContent =
        data.referrals || 0;
}
...
async function loadTelegram() {
    ...
    document.getElementById("telegramAdminUrl").value =
        data.url;
}
...
async function updateTelegramChat() {
    ...
    const url =
        document.getElementById("telegramAdminUrl").value.trim();
...
function appendMessage(message) {
    ...
    if (
        ...
        media =
            \`<img class="media"
            src="\${escapeAttr(message.mediaUrl)}">\`;
    }
    ...
    if (
        ...
        media =
            \`<video controls
            src="\${escapeAttr(message.mediaUrl)}"></video>\`;
    }
    ...
    div.innerHTML = \`
        ...
        <div class="message-body">
            ...
            <div class="message-actions">
                ...
                <button
                class="small-btn"
                onclick="tagUser('\${escapeJs(message.username)}')">
                @ Тегнуть
                </button>
...
function highlightMentions(text) {
    ...
    return text.replace(
        /@([a-zA-Z0-9_.-]+)/g,
        '<span style="color:#00a8ff;font-weight:bold">@$1</span>'
    );
}
    ...
    data.exchangers.forEach(item => {
        ...
        <div class="card">
            ...
            \${
                item.photoUrl
                ?
                \`<img src="\${escapeAttr(item.photoUrl)}">\`
                :
                ""
            }
            ...
            \${
                item.telegramUrl
                ?
                \`<a href="\${escapeAttr(item.telegramUrl)}"
                target="_blank">
                ✈️ Telegram
                </a>\`
                :
...
async function loadAdmins() {
    ...
    data.admins.forEach(admin => {
        ...
        <div class="admin-item">
            ...
            \${
                admin.status !== "main_admin"
                ?
                \`
                <br><br>

                <button
                onclick="editAdmin('\${escapeJs(admin.username)}')">
                ✏️ Изменить
                </button>

                <button
                class="danger"
                onclick="removeAdmin('\${escapeJs(admin.username)}')">
                🗑 Удалить права
                </button>
                \`
                :
...
async function addAdmin() {
    ...
    const username =
        document.getElementById("newAdmin").value.trim();
...
async function removeAdmin(username) {
    ...
    if (!confirm(
        "Удалить права администратора у @" +
        username +
        "?"
    )) return;
...
async function addExchanger() {
    ...
    const r =
        await fetch("/api/exchangers/add", {
            method:"POST",
            headers:{
                "Content-Type":"application/json"
            },
...
async function deleteExchanger(id) {
    ...
    const r =
        await fetch("/api/exchangers/delete", {
            method:"POST",
            headers:{
                "Content-Type":"application/json"
            },
...
async function loadUsers() {
    ...
    allUsers.forEach(user => {
        ...
        <div class="admin-item">
            ...
            \${
                user.username !== "koliaegorov99po-afk"
                ?
                \`
                <br><br>
...
async function loadMyReferrals() {
    ...
    const box =
        document.getElementById("myReferrals");
